...
public class Util {
	private static List<Polygon> findCandidates(int w, int h, double cut, double[][] m1, int buildingsPolyBorder, int buildingsPolyBorder2) {
			...
			if (m3[y][x] == 0) {
				...
				if (tot > minPts) {
					...
					for (int j = 0; j < tot; j++) {
						...
						//if(tot < 100) System.err.print(cx + "," + cy + ";");
						if (eq == 4) continue;
						...
						pts.add(new Point(Math.min(w, cx + buildingsPolyBorder2 + 1), Math.max(0, cy - buildingsPolyBorder2)));
						pts.add(new Point(Math.max(0, cx - buildingsPolyBorder2), Math.min(h, cy + buildingsPolyBorder2 + 1)));
						
						/*temp[cx + w + 4 + (w + 3) * Math.min(h, cy + buildingsPolyBorder + 1)] = id; maxPoint = Math.max(maxPoint, cx + w + 4 + (w + 3) * Math.min(h, cy + buildingsPolyBorder + 1));
						temp[cx + w + 4 + (w + 3) * Math.max(0, cy - buildingsPolyBorder)] = id;
						temp[Math.min(w, cx + buildingsPolyBorder + 1) + w + 4 + (w + 3) * cy] = id; maxPoint = Math.max(maxPoint, Math.min(w, cx + buildingsPolyBorder + 1) + w + 4 + (w + 3) * cy);
						temp[Math.max(0, cx - buildingsPolyBorder) + w + 4 + (w + 3) * cy] = id;
						for(int i = Math.max(0, cx - buildingsPolyBorder2); i <= Math.min(w, cx + buildingsPolyBorder2 + 1); i++) temp[i + w + 4 + (w + 3) * Math.max(0, cy - buildingsPolyBorder2)] = id;
						for(int i = Math.max(0, cx - buildingsPolyBorder2); i <= Math.min(w, cx + buildingsPolyBorder2 + 1); i++) temp[i + w + 4 + (w + 3) * Math.min(h, cy + buildingsPolyBorder2 + 1)] = id;
						maxPoint = Math.max(maxPoint, Math.min(w, cx + buildingsPolyBorder2 + 1) + w + 4 + (w + 3) * Math.min(h, cy + buildingsPolyBorder2 + 1));
						for(int i = Math.max(0, cy - buildingsPolyBorder2) + 1; i < Math.min(h, cy + buildingsPolyBorder2 + 1); i++) temp[Math.max(0, cx - buildingsPolyBorder2) + w + 4 + (w + 3) * i] = id;
						for(int i = Math.max(0, cy - buildingsPolyBorder2) + 1; i <  Math.min(h, cy + buildingsPolyBorder2 + 1); i++) temp[Math.min(w, cx + buildingsPolyBorder2 + 1) + w + 4 + (w + 3) * i] = id;
						*/
					}
					/*ConcaveHull CH = new ConcaveHull();
					ArrayList<ConcaveHull.PointCH> ptsCH = new ArrayList<ConcaveHull.PointCH>();
					for (Point p : pts) {
					  ptsCH.add(new ConcaveHull.PointCH(p.x, p.y));
					}*/
					/*if(tot > 300 && tot < 1000){
					  System.err.println(" ");
					  System.err.println("*");
					  for(Point p : pts) System.err.print(p.x + "," + p.y + ";");
					  System.err.println(" ");
					  System.err.println(" ");
					}*/
					/*List<Integer> cch = new ArrayList<Integer>();
					cch.add(maxPoint);
					int lastPoint = maxPoint;
					int[] dir = new int[] {w + 3, 1, -(w + 3), -1};
					int di = 0;
					while(true){
					  for(int i = di; i < di + 4; i++){
					    if(temp[lastPoint + dir[i % 4]] == id){
					      lastPoint += dir[i % 4];
					      if((di + 1) % 4 == i % 4) cch.set(cch.size() - 1, lastPoint);
					      else cch.add(lastPoint);
					      di = (i + 3) % 4;
					      break;
					    }
					  }
					  if(lastPoint == maxPoint) break;
					}
					cch.remove(cch.size() - 1);
					*/
					/*if(tot > 300 && tot < 1000){
					  System.err.println("**");
					  for(int p : cch) System.err.print((p % (w + 3) - 1) + "," + (p / (w + 3) - 1) + ";");
					  System.err.println(" ");
					}*/
					//List<Point> hull = new ArrayList<Point>();
					//for(int p : cch) hull.add(new Point(p % (w + 3) - 1, p / (w + 3) - 1));
					List<Point> hull = Util.convexHull(pts);
					/*if(tot > 300 && tot < 1000){
					  System.err.println("***");
					  for(Point p : hull) System.err.print(p.x + "," + p.y + ";");
					  System.err.println(" ");
					  System.err.println("*********");
					}*/
					
					//System.err.println(pts.size() + " -> " + hull.size() + "; " + cch.size());
					if (hull != null) {
						...
						//PolygonInfo pa = new PolygonInfo(xp, yp, xp.length, 0);
					}
					/*List<Point> hull2 = Util.convexHull(pts);
					if (hull2 != null) {
						int[] xp = new int[hull2.size()];
						int[] yp = new int[hull2.size()];
						for (int i = 0; i < xp.length; i++) {
							Point p = hull2.get(i);
							xp[i] = p.x;
							yp[i] = p.y;
		...
		/////////////////////////////////////////////////////////////////////////////
		/*
		BufferedImage img2 = new BufferedImage(2 * w, h, BufferedImage.TYPE_INT_BGR);
		Graphics2D g = img2.createGraphics();
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				img2.setRGB(x, y, (int) (255 * Math.max(0, m1[y][x])));
				img2.setRGB(x + w, y, 256 * (int) (255 * Math.max(0, m2[y][x])));
			}
		}
		for (int i = 0; i < candidates.size(); i++) {
			g.setColor(Color.green);
			g.draw(candidates.get(i));
		}
		g.dispose();
		new ImgViewer(img2);
		*/
	...
	public static List<Polygon> findBuildings(double[][] buildingValue, double[][] borderValue, double[][] distValue, double buildingsCut, double borderWeight, int buildingsPolyBorder, int buildingsPolyBorder2) {
		...
		for (int y = 1; y < h - 1; y++) {
			...
			for (int x = 1; x < w - 1; x++) {
				double c = 0;
				for (int ay = y - 1; ay <= y + 1; ay++) {
					c += buildingValue[ay][x + 1] - borderValue[ay][x + 1];
				}
	...
	public static void splitImages(Map<String, List<Building>> buildingsPerImage, int[] a, int idx) {
		try {
			System.err.println("        Splitting Images...");
			long t = System.currentTimeMillis();
			List<String> l = new ArrayList<String>(buildingsPerImage.keySet());
			Random rnd = new Random(14121974);
	...
	public static Map<String, List<Building>> readBuildingsCsv(File trainingCsv) {
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				String image = cols.get(0);
				List<Building> buildings = buildingsPerImage.get(image);
				if (buildings == null) buildingsPerImage.put(image, buildings = new ArrayList<Building>());
	...
	public static double[][][] evalImage(MultiChannelImage mci, RandomForestPredictor buildingPredictor, RandomForestPredictor borderPredictor, RandomForestPredictor distPredictor) {
		try {
			int w = mci.width;
			int h = mci.height;
			double[][] buildingValues = new double[h][w];
			double[][] borderValues = new double[h][w];
			double[][] distValues = new double[h][w];
			int[][] count = new int[h][w];
			for (int y = 0; y + BuildingFeatureExtractor.featRectSize <= h; y += BuildingFeatureExtractor.featTestingStep) {
				for (int x = 0; x + BuildingFeatureExtractor.featRectSize <= w; x += BuildingFeatureExtractor.featTestingStep) {
					float[] features = BuildingFeatureExtractor.getFeatures(mci, x, y, w, h);
					for (int i = 0; i <= 2; i++) {
						double p = (i == 0 ? buildingPredictor : i == 1 ? borderPredictor : distPredictor).predict(features);
						double[][] value = i == 0 ? buildingValues : i == 1 ? borderValues : distValues;
			...
			return new double[][][] {buildingValues,borderValues,distValues};
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(-14);
		}
