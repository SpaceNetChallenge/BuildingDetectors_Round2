...
public class PolygonFeatureExtractor {
	public static final double[] buildingsCuts = new double[] {0.09,0.12,0.15,0.18,0.21,0.24,0.27,0.30,0.33,0.36,0.39,0.42,0.45,0.48,0.51,0.54,0.57,0.60};
	//{0.09, 0.18, 0.25, 0.32, 0.37, 0.44, 0.51, 0.60}; //{0.09,0.15,0.20,0.25,0.29,0.33,0.36,0.40,0.44,0.49,0.54,0.60}; //{0.09,0.12,0.15,0.18,0.21,0.24,0.27,0.30,0.33,0.36,0.39,0.42,0.45,0.48,0.51,0.54,0.57,0.60};
	public static final double[] borderWeights = new double[] {1,0.5};//,2};
	public static final int[] borderShifts = new int[] {0,-1};//,1};
	public static final int buildingsPolyBorder = 5;
	public static final int buildingsPolyBorder2 = 4;

	public static float[] getFeatures(MultiChannelImage mci, Polygon polygon, double[][] buildingValues, double[][] borderValues, double[][] distValues) {
		float[] ret = new float[5 + 6 + 6 * (4 * 2 + 4 * 3 + 2 * 8 * 1)];
		int k = 0;
		...
		List<List<Double>> values3band = new ArrayList<List<Double>>();
		int len = (y1-y0+2)*(x1-x0+2);
		for (int i = 0; i < 8 * 6; i++) {
			values3band.add(new ArrayList<Double>(len));
		}
		len/=4;
		List<List<Double>> values8band = new ArrayList<List<Double>>();
