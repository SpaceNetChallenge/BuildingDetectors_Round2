public class BuildingFeatureExtractor {
	public static final int featRectSize = 2;
	public static final int featTrainingStep = 5; //15;//10;//5;
	public static final int featTestingStep = 1; //2;//1;

	public static float[] getFeatures(MultiChannelImage image, int sx, int sy, int w, int h) {
		float[] ret = new float[5 * (3 * 4 + 2 * 4) + 8 * 2];
		int k = 0;
		for (int i : new int[] {0,1,3,5,7}) {
			int rx = sx - i;
			int ry = sy - i;
			int rs = (i + 1) * 2;
			System.arraycopy(rectStatFeatures(image.h, rx, ry, rs, rs, w, h), 0, ret, k, 3);
			k += 3;
			System.arraycopy(rectStatFeatures(image.s, rx, ry, rs, rs, w, h), 0, ret, k, 3);
			k += 3;
			System.arraycopy(rectStatFeatures(image.l, rx, ry, rs, rs, w, h), 0, ret, k, 3);
			k += 3;
			System.arraycopy(rectStatFeatures(image.edge, rx, ry, rs, rs, w, h), 0, ret, k, 3);
			k += 3;
			System.arraycopy(text(image.l, rx, ry, rs, rs, w, h), 0, ret, k, 2);
			k += 2;
			System.arraycopy(text(image.edge, rx, ry, rs, rs, w, h), 0, ret, k, 2);
			k += 2;
		}
		int w8 = image.w8;
		int h8 = image.h8;
